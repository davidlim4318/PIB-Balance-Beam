...
Y = fft(y-mean(y));  % subtract dc offset
Phiy = abs(Y).^2/N;
w = linspace(0,pi,N/2+1);
...
%% Load Datasets
N = 1200;
data0 = iddata(txt1(1:N,3),txt1(1:N,2),Ts);
data1 = iddata(txt1(N+1:end,3),txt1(N+1:end,2),Ts);
txt2 = readmatrix("expCL4.txt");
data2 = iddata(txt2(:,3),txt2(:,2),Ts);
...
par = [1 0.1];  % ensure no zero-pole cancellation
zF = 0.97;
pF = 0.5;
kF = 1;
