...
const int n = 5;  // maximum order of filters
float U[n + 1];
int Y[n + 1];

const int nP = 4;  // order of prediction filter
float Ypred[nP + 1];

const int nF = 1;  // order of controller filter
int E[nF + 1];

const int nW = 5;  // order of disturbance filter
float Wpred[nW + 1];
...
float bP1[nP + 1] = {0, 0, 0.033931599523769, 0.004083876056261, -0.010524837909073};
float bP2[nP + 1] = {0, 0, 0.760172082168328, -1.048517164030847, 0.304028474727189};
float aP[nP + 1] = {1, -1.859817866121445, 1.191841931896118, -0.316340672910003, 0};
...
float bW1[nW + 1] = {0, 0, -0.009385323309747, 0.003559998954889, 0.009191691215012, -0.003753631049623};
float bW2[nW + 1] = {0.276595369551390, -0.652623879907378, 0.207976520833087, 0.544193637792026, -0.484571890384477, 0.108430242115352};
float aW[nW + 1] = {1, -2.835125237886597, 3.005736006864897, -1.478752895067106, 0.308529390278274, 0};
float disturbanceFilter() {
  float output = 0;
  for (int i = 0; i <= nW; i++) output += bW1[i] * U[i] + bW2[i] * Y[i];
  for (int i = 1; i <= nW; i++) output -= aW[i] * Wpred[i];
  return output;
}
